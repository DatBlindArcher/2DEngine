...
public class CharacterScene extends Scene
{
	...
	{
		super.activate();
		GameObject player = new GameObject("Character", "Character", new Component[] { new CircleCollider(), new Character(), new Image("/Images/Circle.png", 10, new Color(0, 200, 20, 255)) });
		GameObject.create(player, new Vector2(Screen.width / 2, Screen.height / 2), 0f);
		GameObject.create(new GameObject("Main Camera", new Component[] { new CameraFollow(player.transform), new AudioListener() }), new Vector2(Screen.width / 2f, Screen.height / 2f), 0f);
